...
export default function LoginDistributor({ onNavigate, onSetRole }: Props) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-cyan-50 flex flex-col items-center justify-center px-4 font-['Inter',sans-serif] relative overflow-hidden">
      ...
      <div className="absolute top-1/4 right-8 text-6xl opacity-10">🗺️</div>
      ...
      <div className="bg-white rounded-2xl shadow-xl p-8 w-full max-w-md relative z-10">
        ...
        <div className="space-y-4">
          <div>
            ...
            <input type="text" defaultValue="email@contoh.com" className="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm text-gray-600 focus:outline-none focus:ring-2 focus:ring-blue-400" />
          </div>

          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Password</label>
            <div className="relative">
              <input type={showPassword ? 'text' : 'password'} defaultValue="password123" className="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-400 pr-9" />
              <button onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 text-sm">
                {showPassword ? '👁' : '🙈'}
              </button>
            </div>
          </div>
          ...
          <div>
            ...
            <input type="text" defaultValue="REG1234567890" className="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-400" />
          </div>

          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Region/Wilayah</label>
            <div className="relative">
              <select value={region} onChange={e => setRegion(e.target.value)} className="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-400 appearance-none bg-white">
          ...
          <div className="bg-blue-50 rounded-xl p-4 space-y-2">
            <label className="flex items-center gap-2 cursor-pointer">
              ...
              <span className="text-sm font-medium text-gray-700">Hubungkan ke BPJPH (API)</span>
            </label>
            {connectBPJPH && (
              <div>
          ...
          <div className="flex items-center justify-between">
            ...
            <button onClick={() => setShowForgot(true)} className="text-sm text-blue-600 hover:text-blue-700 font-medium underline">Lupa Kata Sandi?</button>
          </div>

          <button onClick={handleLogin} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3.5 rounded-xl text-sm transition-colors">
            Masuk
          </button>

          <p className="text-center text-sm text-gray-500">
            Belum punya akun?{' '}
            <button onClick={() => onNavigate('register')} className="text-blue-600 font-semibold hover:underline">Daftar</button>
          </p>
        </div>
      </div>

      <button onClick={() => onNavigate('landing')} className="mt-6 text-sm text-gray-400 hover:text-gray-600 transition-colors">
        ← Kembali ke Beranda
      </button>
            ...
            {!forgotSent ? (
              <>
                ...
                <div className="flex gap-3">
                  ...
                  <button onClick={() => setForgotSent(true)} className="flex-1 bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2.5 rounded-xl text-sm transition-colors">Kirim Link</button>
                </div>
              </>
            ) : (
              <div className="text-center">
                ...
                <button onClick={() => { setShowForgot(false); setForgotSent(false) }} className="bg-blue-600 hover:bg-blue-700 text-white font-semibold px-6 py-2.5 rounded-xl text-sm transition-colors">Tutup</button>
              </div>
            )}
          </div>
        </div>
