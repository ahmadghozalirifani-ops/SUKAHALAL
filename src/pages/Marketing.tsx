...
const initialCampaigns: { id: number; name: string; type: string; discount: string; start: string; end: string; status: Status; reach: number; orders: number }[] = [
  ...
  { id: 3, name: 'Buy 2 Get 1 Bumbu Nusantara', type: 'Bundle', discount: 'B2G1', start: '15 Feb', end: '28 Feb', status: 'Berakhir', reach: 890, orders: 145 },
]
    ...
    <div className="min-h-screen bg-gray-50 font-['Inter',sans-serif]">
      ...
      <div className="max-w-5xl mx-auto px-6 py-8">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-xl font-extrabold text-gray-900">Marketing & Promosi Halal</h1>
            <p className="text-sm text-gray-500">Kelola kampanye diskon, kupon, dan strategi pemasaran produk halal Anda</p>
          </div>
          <button onClick={() => { setEditId(null); setShowForm(true) }} className="bg-green-600 hover:bg-green-700 text-white font-semibold text-xs px-4 py-2 rounded-xl transition-colors cursor-pointer shadow-sm">
            + Buat Kampanye Baru
          </button>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden mb-6">
          <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
            <h2 className="font-bold text-gray-900 text-sm">Daftar Kampanye Promosi</h2>
          </div>
                  ...
                  <tr key={c.id} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex gap-2">
                        <button onClick={() => handleEdit(c)} className="text-blue-600 hover:underline text-xs font-medium cursor-pointer">Edit</button>
                        {c.status === 'Dijadwalkan' && (
                          <button onClick={() => handleActivate(c.id)} className="text-green-600 hover:underline text-xs font-medium cursor-pointer">Aktifkan</button>
                        )}
                        {c.status === 'Aktif' && (
                          <button onClick={() => handleStop(c.id)} className="text-red-500 hover:underline text-xs font-medium cursor-pointer">Hentikan</button>
                        )}
                      </div>
                    </td>
                  </tr>
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-2xl p-5 shadow-xs">
          <h3 className="font-bold text-amber-800 mb-3 flex items-center gap-2 text-sm"><span>💡</span> Tips Pemasaran Produk Halal UMKM</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {[
              'Tampilkan badge "Halal Tayiban" secara mencolok di setiap materi promosi',
              'Buat kampanye tematik di momen Ramadan, Idul Fitri, dan Hari Santri',
              'Tawarkan paket bundle produk halal yang saling melengkapi',
              'Cantumkan nomor sertifikasi BPJPH resmi untuk meningkatkan trust konsumen',
            ].map((tip, i) => (
      ...
      {showForm && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-md">
            <h2 className="font-extrabold text-gray-900 text-base mb-5">{editId ? 'Edit Kampanye' : 'Buat Kampanye Baru'}</h2>
            <div className="space-y-4">
              <div>
                <label className="text-xs font-medium text-gray-700 block mb-1">Nama Kampanye</label>
                <input type="text" value={newCampaign.name} onChange={e => setNewCampaign({ ...newCampaign, name: e.target.value })}
                  placeholder="e.g. Promo Berkah Ramadan"
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs font-medium text-gray-700 block mb-1">Tipe Promosi</label>
                  <select value={newCampaign.type} onChange={e => setNewCampaign({ ...newCampaign, type: e.target.value })}
                    className="w-full border border-gray-200 rounded-xl px-3 py-2 text-xs focus:outline-none focus:ring-2 focus:ring-green-400 bg-white">
                    {['Diskon Produk', 'Flash Sale', 'Bundle', 'Voucher'].map(t => <option key={t}>{t}</option>)}
                  </select>
                </div>
                <div>
                  <label className="text-xs font-medium text-gray-700 block mb-1">Besar Diskon</label>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="text-xs font-medium text-gray-700 block mb-1">Tanggal Berakhir</label>
                  <input type="date" value={newCampaign.end} onChange={e => setNewCampaign({ ...newCampaign, end: e.target.value })}
                    className="w-full border border-gray-200 rounded-xl px-3 py-2 text-xs focus:outline-none focus:ring-2 focus:ring-green-400" />
                </div>
              </div>
              <div className="flex gap-3 pt-2">
                <button onClick={() => { setShowForm(false); setEditId(null) }} className="flex-1 border border-gray-200 text-gray-600 font-semibold py-2.5 rounded-xl text-xs hover:bg-gray-50 cursor-pointer">Batal</button>
                <button onClick={handleCreate} className="flex-1 bg-green-600 hover:bg-green-700 text-white font-semibold py-2.5 rounded-xl text-xs transition-colors cursor-pointer shadow-sm">
