...
const sellerNav = [
  ...
  { id: 'product-management', label: 'Kelola Produk (CRUD)', icon: '🛠️' },
...
const customerNav = [
  ...
  { id: 'tutorial', label: 'Tutorial & Edukasi', icon: '📖' },
  { id: 'settings', label: 'Pengaturan', icon: '⚙️' },
]
...
const customerStats = [
  ...
  { label: 'Wishlist Favorit', value: '12', color: 'text-pink-500', highlight: false },
...
const activities = [
  { icon: '📄', text: 'Dokumen diunggah oleh', bold: 'PT Berkah Jaya', time: '25 menit lalu', color: 'text-blue-500' },
  { icon: '✅', text: 'Verifikasi disetujui untuk', bold: 'Produk Mie Halal', time: '25 menit lalu', color: 'text-green-500' },
  { icon: '✅', text: 'Verifikasi diajukan untuk', bold: 'Produk Ayam Beku', time: '1 jam lalu', color: 'text-green-500' },
  { icon: '⏳', text: 'Review BPJPH pending untuk', bold: 'Sambal Organik', time: '2 jam lalu', color: 'text-amber-500' },
]
...
export default function Dashboard({ onNavigate, userRole }: Props) {
  ...
  // Guest view (without login) focuses on education & safe general information
  if (userRole === 'guest') {
    return (
      <div className="min-h-screen bg-gray-50 font-['Inter',sans-serif]">
        <header className="h-16 bg-white border-b border-gray-100 flex items-center justify-between px-8 sticky top-0 z-40 shadow-sm">
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-1 text-xs text-gray-600 bg-gray-50 px-2.5 py-1 rounded-full border border-gray-200">
              <button onClick={() => i18n.changeLanguage('id')} className={lang === 'ID' ? 'font-bold text-green-700' : 'text-gray-400 hover:text-gray-600'}>ID</button>
              <span className="text-gray-300">|</span>
              <button onClick={() => i18n.changeLanguage('en')} className={lang === 'EN' ? 'font-bold text-green-700' : 'text-gray-400 hover:text-gray-600'}>EN</button>
            </div>
            <button onClick={() => onNavigate('landing')} className="text-sm text-gray-600 hover:text-gray-900 font-medium">← {t('common.back', 'Beranda')}</button>
            <button onClick={() => onNavigate('login-seller')} className="bg-green-600 hover:bg-green-700 text-white text-xs font-semibold px-4 py-2 rounded-lg transition-colors shadow-sm">
        ...
        <main className="max-w-5xl mx-auto p-6 md:p-8 space-y-6">
          <div className="bg-gradient-to-r from-green-600 to-teal-600 rounded-2xl p-8 text-white shadow-md">
            <span className="bg-white/20 text-xs px-3 py-1 rounded-full font-semibold uppercase tracking-wider">Edukasi Rantai Pasok Halal</span>
            <h1 className="text-3xl font-extrabold mt-3 mb-2">Transparansi Halal dari Hulu ke Hilir</h1>
            <p className="text-green-100 text-sm max-w-2xl leading-relaxed">
              SUKAHALAL memastikan setiap produk yang sampai ke tangan Anda telah melewati pengawasan ketat, mulai dari petani bahan baku, produsen, pengemasan bebas kontaminasi, hingga logistik bersertifikasi BPJPH.
            </p>
            <div className="mt-6 flex flex-wrap gap-3">
              <button onClick={() => onNavigate('product-catalog')} className="bg-white text-green-700 font-bold px-6 py-2.5 rounded-xl text-sm hover:bg-green-50 transition-colors shadow">
                🛍️ Jelajahi Produk Halal
              </button>
              <button onClick={() => onNavigate('tutorial')} className="bg-green-700/60 border border-green-400/40 text-white font-medium px-5 py-2.5 rounded-xl text-sm hover:bg-green-700 transition-colors">
                📖 Pelajari Alur Sertifikasi
              </button>
            </div>
          </div>

          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
            <h2 className="font-bold text-gray-900 text-base mb-2">Simulasi Alur Rantai Pasok Terbuka</h2>
            <p className="text-xs text-gray-500 mb-4">Setiap tahapan diverifikasi secara digital untuk mencegah pencampuran dengan bahan non-halal.</p>
            <SupplyChainFlow />
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
              ...
              <h3 className="font-bold text-gray-900 text-sm mb-1">Halal & Toyyiban</h3>
              <p className="text-xs text-gray-500 leading-relaxed">Bukan hanya sah secara syariat, tetapi juga bersih, higienis, sehat, dan aman dikonsumsi oleh seluruh kalangan.</p>
            </div>
            <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
              <div className="text-3xl mb-2">🔍</div>
              <h3 className="font-bold text-gray-900 text-sm mb-1">Lacak Sertifikat BPJPH</h3>
              <p className="text-xs text-gray-500 leading-relaxed">Seluruh nomor sertifikat terhubung langsung dengan sistem SIHALAL BPJPH dan dapat diverifikasi keasliannya.</p>
            </div>
            <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
              <div className="text-3xl mb-2">🚚</div>
              <h3 className="font-bold text-gray-900 text-sm mb-1">Logistik Terpisah</h3>
              <p className="text-xs text-gray-500 leading-relaxed">Armada pengiriman menjamin segregasi fisik agar produk halal tidak bersentuhan dengan zat yang meragukan.</p>
            </div>
          </div>
        </main>
      </div>
  ...
  const userName = userRole === 'seller' ? 'Ahmad Seller' : userRole === 'distributor' ? 'PT Distribusi Halal' : 'Budi Customer'
    ...
    <div className="flex h-screen bg-gray-50 font-['Inter',sans-serif] overflow-hidden">
      ...
      <aside className="w-56 bg-white border-r border-gray-100 flex flex-col shrink-0">
        <div className="flex items-center gap-2 px-5 py-5 border-b border-gray-100 cursor-pointer" onClick={() => onNavigate('landing')}>
          <div className="w-8 h-8 bg-green-600 rounded-lg flex items-center justify-center text-white font-extrabold text-sm shadow-sm">S</div>
          <span className="font-extrabold text-green-700 text-sm tracking-tight">SUKAHALAL</span>
        </div>
        ...
        <div className="px-4 py-3 border-t border-gray-100">
          <button onClick={() => onNavigate('logout')} className="w-full flex items-center gap-2 px-3 py-2 rounded-lg text-sm text-red-500 hover:bg-red-50 transition-colors font-medium cursor-pointer">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <header className="h-14 bg-white border-b border-gray-100 flex items-center justify-between px-6 shrink-0">
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={() => onNavigate('notifikasi')} className="relative w-9 h-9 rounded-full bg-gray-50 hover:bg-gray-100 flex items-center justify-center text-gray-500 transition-colors cursor-pointer">
              🔔
            ...
            <div className="relative">
              <button onClick={() => setShowProfileMenu(!showProfileMenu)} className="flex items-center gap-2 cursor-pointer hover:bg-gray-50 rounded-lg px-2 py-1 transition-colors">
                <div className="w-8 h-8 rounded-full bg-amber-200 flex items-center justify-center text-sm shadow-sm">
              ...
              {showProfileMenu && (
                <div className="absolute right-0 top-full mt-1 bg-white rounded-xl shadow-xl border border-gray-200 w-48 py-1 z-50">
                  ...
                  <button onClick={() => { onNavigate('notifikasi'); setShowProfileMenu(false) }} className="w-full text-left px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50 flex items-center gap-2">
                    🔔 {t('nav.notifications', 'Notifikasi')}
                  </button>
        ...
        <main className="flex-1 overflow-y-auto p-6">
          ...
          <div className="mb-4">
            <span className={`text-xs font-semibold px-2.5 py-1 rounded-full ${
              userRole === 'seller' ? 'bg-green-100 text-green-700 border border-green-200' : userRole === 'distributor' ? 'bg-blue-100 text-blue-700 border border-blue-200' : 'bg-purple-100 text-purple-700 border border-purple-200'
            }`}>
              {userRole === 'seller' ? '🛒 Mode Penjual (Seller UMKM)' : userRole === 'distributor' ? '🚛 Mode Distributor' : '👤 Mode Konsumen'}
            </span>
          </div>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            {stats.map((s, i) => (
              <div key={i} className={`bg-white rounded-2xl p-5 border ${s.highlight ? 'border-green-200 bg-green-50/20' : 'border-gray-100'} shadow-sm`}>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
              <div className="font-semibold text-gray-800 mb-4 text-sm">Aksi Cepat</div>
                ...
                {userRole === 'customer' ? (
                  <>
                    {[
                      ...
                      <button key={i} onClick={() => onNavigate(a.page)}
                        className="bg-purple-600 hover:bg-purple-700 text-white w-full text-sm font-semibold py-3 rounded-xl transition-colors flex items-center gap-2 px-4 shadow-sm cursor-pointer">
                  ...
                  <>
                    {[
                      ...
                      <button key={i} onClick={() => onNavigate(a.page)}
                        className="bg-blue-600 hover:bg-blue-700 text-white w-full text-sm font-semibold py-3 rounded-xl transition-colors flex items-center gap-2 px-4 shadow-sm cursor-pointer">
                        <span>{a.icon}</span> {a.label}
                      </button>
                    ))}
                  </>
                  ...
                  <>
                    {[
                      { label: 'Kelola & Tambah Produk', icon: '🛠️', page: 'product-management' },
                      { label: 'Upload Dokumen BPJPH', icon: '⬆', page: 'upload-dokumen' },
                      { label: 'Lihat Pesanan Masuk', icon: '🛒', page: 'pesanan' },
                    ].map((a, i) => (
                      <button key={i} onClick={() => onNavigate(a.page)}
                        className="bg-green-600 hover:bg-green-700 text-white w-full text-sm font-semibold py-3 rounded-xl transition-colors flex items-center gap-2 px-4 shadow-sm cursor-pointer">
                        <span>{a.icon}</span> {a.label}
                      </button>
                    ))}
                  </>
              ...
              <>
                {[
                  ...
                  <button key={i} onClick={() => onNavigate(s.page)}
                    className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 hover:shadow-md transition-shadow text-left cursor-pointer">
          ...
          <div className="mt-5 bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
            <div className="flex items-center justify-between mb-4">
              <div className="font-semibold text-gray-800 text-sm">Aktivitas Terkini</div>
