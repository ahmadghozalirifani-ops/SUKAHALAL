...
export default function VerificationCenter({ onNavigate }: Props) {
  ...
  function handleNav(id: string) {
    setActiveNav(id)
    // map 'logistics' to inventaris since we don't have a logistics page
    const target = id === 'logistics' ? 'inventaris' : id
    if (target !== 'verification') onNavigate(target)
  }
  ...
  return (
    <div className="flex h-screen bg-gray-50 font-['Inter',sans-serif] overflow-hidden">
      ...
      <main className="flex-1 overflow-y-auto">
        <div className="bg-white border-b border-gray-100 px-6 py-3 flex items-center justify-end">
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 rounded-full bg-gray-100 flex items-center justify-center text-gray-500 text-sm">👤</div>
        ...
        <div className="p-6">
          <div className="mb-6">
            <h1 className="text-xl font-extrabold text-gray-900">Pusat Verifikasi Halal</h1>
            <p className="text-sm text-gray-500 mt-1">Kelola dan pantau proses verifikasi sertifikasi Halal Anda secara efisien.</p>
          </div>
          ...
          <div className="flex gap-0 mb-6 overflow-x-auto">
            {steps.map((step, i) => (
              <button key={step.num} onClick={() => {
                ...
                className={`flex-1 min-w-36 flex flex-col items-center gap-2 py-4 px-4 relative transition-colors ${
                  activeStep === step.num ? 'bg-teal-700 text-white' : 'bg-gray-100 text-gray-500 hover:bg-gray-200'
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm">
            <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
              ...
              <button onClick={() => onNavigate('upload-dokumen')} className="bg-teal-600 hover:bg-teal-700 text-white text-xs font-semibold px-4 py-2 rounded-lg transition-colors">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {items.map((item, i) => (
                    <tr key={item.no} className="border-b border-gray-50 hover:bg-gray-50/50">
                      <td className="px-5 py-3.5 text-gray-600">{item.no}</td>
                      <td className="px-5 py-3.5 font-medium text-gray-800">{item.product}</td>
                      <td className="px-5 py-3.5 text-gray-600">{item.submitted}</td>
                      ...
                      <td className="px-5 py-3.5 text-gray-600">{item.eta}</td>
                      <td className="px-5 py-3.5">
                        <div className="flex items-center gap-2">
                          <button onClick={() => onNavigate('product-detail')} className="text-blue-600 hover:text-blue-700 text-xs font-medium">Lihat Detail</button>
                          <span className="text-gray-300">|</span>
                          {item.status === 'Ditolak' ? (
                            <button onClick={() => onNavigate('upload-dokumen')} className="text-orange-600 hover:text-orange-700 text-xs font-medium">Re-submit</button>
                          ) : (
            ...
            <div className="flex items-center justify-center gap-2 py-4 border-t border-gray-100">
              {['⟨⟨', '⟨', '1', '...', '⟩', '⟩⟩'].map((n, i) => (
                <button key={i} className={`w-8 h-8 rounded-lg text-sm font-medium ${n === '1' ? 'bg-teal-600 text-white' : 'border border-gray-200 text-gray-500 hover:bg-gray-50'}`}>{n}</button>
              ))}
            </div>
          </div>
        </div>
